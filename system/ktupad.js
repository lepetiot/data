...
out  = '<div id="cube"></div>';
out  += '<form id="Form" autocomplete="off" >';
// for (i in fld) {key=fld[i]; dan=k[x].table.hidden(key);
  for (key in fld) { tid=fld[key]; dan=k[x].table.hidden(key);
debug(fld[key]);
out += '<div class="row '+dan+'">';
out += '<div id="f-'+key+'" style="margin:5px">';
out += '<div id="la-'+key+'" class="l-4 s-12">';
...
out += '<div class="l-8 s-12">';
out += '<div id="el-'+key+'">';
// out +='<input type="'+typ+'" id="'+key+'" name="'+key+'" value="">';
out += k[x].form.inputan(tid,key);
out +='</div>';
...
inputan:function(tid,key){ debug('form.inputan');
typ=typedata[tid];
out = '<input type="'+typ+'" id="'+key+'" name="'+key+'" value="">';
if(tid==252){ out='<textarea name='+key+' id='+key+' ></textarea>';}
return out;
},
...
out +='<th class="noprint pColor '+isheader+'" ><input type="checkbox" name="chs" onclick=k.'+x+'.table.CheckAll(this,\'chk'+x+'\')></th>';
...
for (var i = (this.page-1) * this.rpp; i < (this.page * this.rpp) && i < al; i++) {cols=data[i];
out += '<tr>';
out += '<td class="noprint '+isheader+'"><input type="checkbox" name="chk'+x+'" value="'+i+'" onclick=k.'+x+'.table.getVal(x)>  </td>';
